...
export default function FlipCardComponent({ items = [] }) {
  ...
  // CSS để ẩn scrollbar nhưng vẫn cho phép scroll
  useEffect(() => {
    // Thêm style để ẩn scrollbar nhưng vẫn giữ chức năng scroll
  ...
  return (
    // Thay đổi từ grid sang flex cho mobile và thêm overflow-x-auto để scroll ngang
    <div className="flex sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4 m-auto w-full max-w-7xl px-4 py-6 overflow-hidden">
      {/* Thêm div container bên ngoài cho mobile với CSS tối ưu scroll ngang mượt mà */}
      <div className="flex flex-nowrap overflow-x-auto sm:contents w-full snap-x snap-mandatory hide-scrollbar touch-pan-x pb-8">
        {displayItems.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`react-card-flip flex-shrink-0 w-[90%] sm:w-full max-w-xs sm:max-w-full mx-3 first:ml-0 last:mr-0 sm:mx-auto h-[650px] sm:h-auto md:h-[44rem] max-h-screen transition-all duration-1000 ease-out snap-center
                        ${animationState >= 1 ? 'opacity-100' : 'opacity-0'}
                        ${animationState >= 1 ? 'scale-100' : 'scale-90'}`}
              style={{
                perspective: "1000px",
                ...
                transitionDelay: `${index * 200}ms`,
                transformOrigin: "center center",
                WebkitOverflowScrolling: "touch" // Cải thiện scroll trên iOS
              }}
            >
              <div
                className="react-card-flipper"
